...
{		
	...
	for (i = 0; i < message_size; i++){
		crc ^= (unsigned short)(message[i]); // Get next byte
		for (j = 7; j >= 0; j--){ // Do eight times
			crc = (crc >> 1) ^ (0xEDB8 & (unsigned short)-(crc & 1));
		}
	}
	return ~crc;
}
...
start:
	for (i = 0; i < DFS_META_NUM_CHUNKS / BITS_PER_UINT; i++){
		if (bm2->ints[i] > 0){
			group = i;
			bit = ffs(bm2->ints[group]) - 1;
			idx = bit + group * BITS_PER_UINT;
			if (unlikely(bm2 == dfs->chunk_bitmaps + DFS_META_FREE_BITMAP)){
				...
				bm2->ints[group] &= ~(1 << bit);
				bm->ints[group] |= 1 << bit;
				dfs->heads[idx].capacity |= (unsigned long)sz_index << META_LIST_HEAD_INDEX_START_BIT;
			}
			goto found_chunk;
		}
	}
...
found_chunk:
	...
	head = dfs->heads + idx;
	chunk = dfs->chunks + idx * dfs->chunk_size;
...
static inline int dfs_dentry_extra_for_len(size_t namelen){
	return (namelen + DFS_META_SIZE_GRANULARITY - DFS_DENTRY_INLINE_LEN) / DFS_META_SIZE_GRANULARITY;
}
...
static inline struct imeta* get_imeta(metaidx_t idx)
{
	return &(dfs->imeta.arr[idx / DFS_IMETA_PER_LIST_NODE].imeta[idx % DFS_IMETA_PER_LIST_NODE]);
}

static inline struct imeta* imeta_dup_from_orig(struct imeta* im)
{
	return (struct imeta*)((char*)(dfs->imeta.dup_arr) + (uint64_t)im - (uint64_t)(dfs->imeta.arr));
}

// Caller must hold imeta lock
static inline void imeta_change_refs_dup(struct imeta* im, struct imeta* im_dup, int i){
	im->refs += i;
	im->chksum = crc16b((unsigned char*)&im->chksum, offsetof(struct imeta, chksum));
	im_dup->refs += i;
	im_dup->chksum = crc16b((unsigned char*)&im_dup->chksum, offsetof(struct imeta, chksum));
}

// Caller must hold imeta lock
static inline void imeta_change_refs(struct imeta* im, int i){
	imeta_change_refs_dup(im, imeta_dup_from_orig(im), i);
}

static inline void read_imeta(const struct dfs_inode* inode, kuid_t* uid, kgid_t* gid, umode_t* mode)
{
	struct imeta* p;
	p = get_imeta(dfs_inode_get_meta_idx(inode));
	if (uid)
		*uid = p->uid;
	if (gid)
		*gid = p->gid;
...
{
	return S_ISDIR(get_imeta(dfs_inode_get_meta_idx(inode))->mode);
}
...
{
	return S_ISREG(get_imeta(dfs_inode_get_meta_idx(inode))->mode);
}
...
static void free_inode(struct dfs_inode* inode)
{
	...
	dfs_imeta_lock(dfs);
	imeta_change_refs(get_imeta(idx), -1);
...
static inline metaidx_t imeta_hash(umode_t mode, kuid_t uid, kgid_t gid)		
{	
    unsigned int merge_id = uid.val >= gid.val ? uid.val * uid.val + uid.val + gid.val : uid.val + gid.val * gid.val;		
    unsigned int hash = (mode + merge_id) * (mode + merge_id + 1) / 2 + mode;		
    return (metaidx_t)(((hash >> 16) ^ ((hash & 0xffff) * 2654435761)) % (dfs->imeta.sz - 1) + 1);		
}

static metaidx_t find_imeta_idx(umode_t mode, kuid_t uid, kgid_t gid, struct imeta* old)
{
	...
	while (prev != &curr->next){
		...
		if (unlikely(imcount == 0)){ // empty node; remove
			if (curr->next == curr - dfs->imeta.arr){ // curr's next points to itself
				if (curr == orig){ // if chain is singular
					im = orig->imeta;
					goto found_new;
				}
				*prev = ((char*)prev - (char*)(dfs->imeta.arr)) / sizeof(struct imeta_list_node); // point prev towards itself
			}
			else{
				*prev = curr->next; // point prev to next
			}
...
found_new:
	...
	im_dup->mode = mode;
out:
	imeta_change_refs_dup(im, im_dup, 1);
	if (old)
		imeta_change_refs(old, -1);
	dfs_imeta_unlock(dfs);
	return idx;
}
...
static struct dfs_inode* create_inode(umode_t mode, struct dfs_inode* dir_parent)
{
	...
	idx = find_imeta_idx(mode, uid, gid, NULL);
	if (idx == 0){
		if (S_ISDIR(mode)){
	...
	//inode->__lock_metaidx_size = 0; // memset in dfs_meta_alloc already does this
...
static void inode_to_kstat(const struct dfs_inode* inode, struct kstat* st)
{
	...
	read_imeta(inode, &st->uid, &st->gid,
	           &st->mode);
	st->rdev = 0;
...
ssize_t dfs_pread(struct dfs_file* file, void __user* buf, size_t count, off_t off)
{
	ssize_t ret;
	struct dfs_inode* inode;
...
{
	return dfs_pread(file, buf, count, -1);
}
...
/* off == -1 means use and update offset of fd */
ssize_t dfs_pwrite(struct dfs_file* file, const void __user* buf, size_t count, off_t off)
{
	ssize_t ret;
	struct dfs_inode* inode;
	...
	ret = dfs_inode_write(inode, off == -1? file->offset : off, buf, count);
	if (ret > 0 && off == -1)
		file->offset += ret;
...
{
	return dfs_pwrite(file, buf, count, -1);
}
                     ...
                     struct linux_dirent __user* dirp, unsigned int count)
{
	...
	char d_type = isdir(dent->inode) ? DT_DIR : DT_REG; /* FIXME if other types added */
                       ...
                       struct dfs_inode* nstart, const char* newpath)
{
	...
	if (!IS_ERR(victim)) { // Replace inode
		//ret = dfs_remove_dent(newpdir, newname);
		newdent = dir_lookup(newpdir, newname, NULL);
		if (unlikely(!newdent)){
			...
			newdent->inode = inode;
			dfs_inc_nlink(inode);
			dfs_dec_nlink(victim);
			dfs_unpin_inode(victim);
			drop_dentry(newdent); // balance dir_lookup
		}
		//if (unlikely(ret))
		//	goto out_unlock_dirs;
		...
		if (unlikely(oldpdir == newpdir) && dfs_dentry_extra_for_len(newlen) == dfs_dentry_extra_for_len(oldlen)){
			// same directory, same cache line size: reuse dentry
			newdent = dir_lookup(newpdir, oldname, NULL);
			if (likely(newdent)){
				newdent->namelen = newlen;
				strcpy(newdent->name, newname);
				dfs_inode_size_add(newpdir, newlen - oldlen);
				drop_dentry(newdent); // balance dir_lookup
				ret = 0;
				goto out_unlock_dirs;
			}
...
int change_imeta(struct dfs_inode** inode, uid_t uid, gid_t gid, umode_t* mode) // double pointer to avoid dereferencing file outside the dfs lock
{
	...
	if (kuid.val != im->uid.val || kgid.val != im->gid.val || *mode != im->mode){
		new = find_imeta_idx(*mode, kuid, kgid, im);
		if (new > 0)
