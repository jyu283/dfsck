...
void print_usage()
{
    printf("Usage: corrupt_dentries image num_of_dentries_to_corrupt\n");
}
...
int main(int argc, char *argv[])
{
    ...
    //print_metadata();
    //corrupt_metadata();
    struct dfs_dentry *dentry_to_clear;
    int num_dentries_corrupt = atoi(argv[2]);
    if(num_dentries_corrupt > dentries_cnt){
        num_dentries_corrupt = dentries_cnt;
    }
    for(int i = 0; i < num_dentries_corrupt; i++){
        dentry_to_clear = dentries[i];
        // size isnt perfect but this should be fine
        memset(dentry_to_clear, 0x0, FILE_INODE_SZ);
    }
    dfscorrupt_free();
