...
void print_usage()
{
    printf("Usage: corrupt_file_inodes image num_of_file_inodes_to_corrupt\n");
}
...
int main(int argc, char *argv[])
{
    ...
    //print_metadata();
    //corrupt_metadata();
    struct dfs_inode *inode_to_clear;
    int num_files_corrupt = atoi(argv[2]);
    if(num_files_corrupt > file_cnt){
        num_files_corrupt = file_cnt;
    }
    for(int i = 0; i < num_files_corrupt; i++){
        inode_to_clear = files[i];
        memset(inode_to_clear, 0x0, FILE_INODE_SZ);
    }
    dfscorrupt_free();
